\begin{tikzpicture}[node distance = 4cm, auto]
    ...
    \node [block, below of=orbit-params] (sdp) {Модель\\ движения\\ КО};
	\node [block, below of=ionosphere-params] (nequick) {Модель\\ влияния\\ ионосферы};
	\node [block, below left of=nequick, node distance=5cm] (radiowave) {Модель\\ радиолокационных\\ измерений};
	\node [block, left of=radiowave, node distance=6cm] (error) {Модель\\ ошибок};
	\node [input, below left of=radiowave, node distance=4cm] (radar-params) {Параметры\\ РЛС};
