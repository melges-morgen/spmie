...
\node [block, right of=TLEReader, node distance = 6cm]      (NeQuick)           {NeQuick};
\node [block, right of=AstroUtils, node distance = 6cm]     (RadarStation)      {RadarStation};
\node [block, right of=OrbitCalculator, node distance = 6cm](out)               {Выход};
\node [input, below of=out, node distance=2.5cm]              (end)               {Конец итерации};
